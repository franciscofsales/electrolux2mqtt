...
export class HomeAssistantService {
  ...
  private async registerDevice(appliance: ElectroluxApiResponse): Promise<void> {
    ...
    try {
      ...
      const deviceInfo = {
        ...
        model: appliance.info.modelName || 'Unknown',
        sw_version: '1.0.0',
  ...
  private async createSensors(
    ...
    // If the device is a washer/dryer or dishwasher, create program status sensor
    if (
      ...
      await this.createSensor({
        ...
        stateTopic,
        valueTemplate: '{{ value_json.program }}',
      ...
      if (state.remainingTime !== undefined) {
        await this.createSensor({
          ...
          stateTopic,
          valueTemplate: '{{ value_json.remainingTime }}',
